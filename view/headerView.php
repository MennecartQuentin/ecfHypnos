<header>
    <div>
        <a href="/">
            <img src="/content/img/logos/logohypnos.png" alt="Hypnos logo" />
            <h3>Hôtel</h3>
            <h2 id="branding-title--rotate">Les</h2>
            <h1>Hypnos</h1>
            <div></div>
        </a>
    </div>
    <nav id="nav-primary">
        <ul>
            <li><a href="/hotel" class="nav-primary__link">L'Hôtel</a></li>
            <li>
                <a href="/bedrooms" class="nav-primary__link">Nos chambres</a>
                <ul>
                    <li><a href="/bedrooms#simple-room" class="nav-primary__link-our-rooms__sub-menu">Chambre simple</a></li>
                    <li><a href="/bedrooms#double-room" class="nav-primary__link-our-rooms__sub-menu">Chambre double</a></li>
                    <li><a href="/bedrooms#twin-room" class="nav-primary__link-our-rooms__sub-menu">Chambre twin</a></li>
                </ul>
            </li>
            <li><a href="/booking/connect.php" class="nav-primary__link">Réservations</a></li>
            <li>
                <a href="/about" class="nav-primary__link">&Agrave; propos de nous</a>
                <ul>
                    <li><a href="/directors-message" class="nav-primary__link-who__sub-menu">Le mot du directeur</a></li>
                    <li><a href="/team" class="nav-primary__link-who_sub-menu">L'équipe</a></li>
                </ul>
            </li>
            <li><a href="/ecfHypnos/contact.php" class="nav-primary__link">Contact</a></li>
            <li><a href="/ecfHypnos/login.php" class="nav-primary__link" id="nav-primary__login"></a></li>
        </ul>
    </nav>
</header>
